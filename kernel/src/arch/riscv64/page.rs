...
impl<T: VirtToPhys> VirtToPhys for &T {
    type Error = T::Error;
    #[inline(always)]
...
impl<T: PhysToVirt> PhysToVirt for &T {
    type Error = T::Error;
    #[inline(always)]
    ...
    fn virt_to_phys<P>(&self, virt: VirtPtr<P>) -> Result<PhyPtr<P>, Self::Error> {
        ...
        let root = unsafe {
            &*self
                .root
                .to_virt(&self.trans)
                .map_err(PageTableAddressTranslationError::PhysicalToVirtualTranslationError)?
                .0
        };
        ...
        if lvl1.is_leaf() {
            let addr = extract_bits(lvl1.ppn() << 12, 12 + 9 + 9, 9)
                | extract_bits(virt as u64, 0, 12 + 9 + 9);
            return Ok(PhyPtr(addr as *mut P));
        }

        let curr = unsafe {
            &*PhyPtr((lvl1.ppn() << 12) as *mut PageTable)
                .to_virt(&self.trans)
                .map_err(PageTableAddressTranslationError::PhysicalToVirtualTranslationError)?
                .0
        };
        ...
        if lvl2.is_leaf() {
            let addr = extract_bits(lvl1.ppn() << 12, 12 + 9 + 9, 9 + 9)
                | extract_bits(virt as u64, 0, 12 + 9);
            return Ok(PhyPtr(addr as *mut P));
        }

        let curr = unsafe {
            &*PhyPtr((lvl2.ppn() << 12) as *mut PageTable)
                .to_virt(&self.trans)
                .map_err(PageTableAddressTranslationError::PhysicalToVirtualTranslationError)?
                .0
        };
        ...
        if lvl3.is_leaf() {
            let addr =
                extract_bits(lvl1.ppn() << 12, 12, 9 + 9 + 9) | extract_bits(virt as u64, 0, 12);
...
impl PageTableEntry {
    ...
    pub const DIRTY_ACCESSED: PageTableEntry =
        PageTableEntry::new().set_dirty(true).set_accessed(true);

    pub const CACHEABLE_MEM: PageTableEntry = PageTableEntry::new()
        .set_cacheable(true)
        .set_bufferable(true);
    pub const NON_CACHEBLE_MEM: PageTableEntry = PageTableEntry::new()
        .set_bufferable(true)
        .set_shareable(true);

    pub const BUFFERABLE_DEV: PageTableEntry = PageTableEntry::new()
        .set_bufferable(true)
        .set_strong_order(true)
        .set_shareable(true);
    pub const NON_BUFFERABLE_DEV: PageTableEntry = PageTableEntry::new()
        .set_strong_order(true)
        .set_shareable(true);
