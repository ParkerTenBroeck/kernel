...
pub fn init(dtb: &Dtb) -> Result<(), DtbError> {
    ...
    {
        let node = dtb.root()?
            .find_compatable_nodes(b"syscon-poweroff")?
            .expect_one()?;
        ...
        let node = dtb.root()?.find_compatable_nodes(b"syscon")?.expect_one()?;
        let props = node.properties()?;
    ...
    {
        let node = dtb.root()?.find_compatable_nodes(b"syscon-reboot")?.expect_one()?;
        let props = node.properties()?;
        let handle = props.expect(b"regmap")?.u32()?;
        let offset = props.expect(b"offset")?.u32()? as usize;
        let value = props.expect(b"value")?.u32()?;
