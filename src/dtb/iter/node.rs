...
pub struct DtbNodeIter<'a>(pub DtbStructParser<'a>);
...
impl<'a> Iterator for DtbNodeIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        loop {
            match self.0.next()? {
                DtbToken::BeginNode(name) if level == 0 => return Some(DtbNode::new(name, self.0)),
                DtbToken::BeginNode(_) => level += 1,
                DtbToken::EndNode if level == 0 => return None,
                DtbToken::EndNode => level -= 1,
                DtbToken::Prop(_) => {}
                DtbToken::Nop => {}
            }
        }
