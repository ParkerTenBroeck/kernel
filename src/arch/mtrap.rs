use riscv::register::{mtvec::Mtvec, satp::Mode};
...
core::arch::global_asm!(
    ...
    mtrap_vector:
        ...
        beq t0, x0, m.handle_synchronous		/* Branch past interrupt handing if not asynchronous. */
        	

    m.handle_asynchronous:
        sd a2, 0( sp )
        jal {handler}
        j m.return

    m.handle_synchronous:
        addi t0, a2, 4
        sd t0, 0( sp )
        jal {handler}
...
// static PLIC_HANDLERS: [AtomicPtr<()>; plic::MAX_INT_ID] =
//     [const { AtomicPtr::new(core::ptr::null_mut()) }; plic::MAX_INT_ID];
...
pub fn init(dtb: &dtb::Dtb) {
    println!("Enabling M-Mode interrupts");

    unsafe extern "C" {
        #[link_name = "mtrap_vector"]
        pub fn mtrap_vector();
    }
    unsafe {
        riscv::register::mtvec::write(Mtvec::new(
            mtrap_vector as *const () as usize,
    ...
    println!("Interrupts enabled M-Mode");
}
